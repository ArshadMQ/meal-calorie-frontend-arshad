...
export function LoginForm({ className, ...props }: React.ComponentProps<"form">) {
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (result?.error) {
        toast(result.error, {
          description: "There is something wrong with your credentials. Please try again.",
        });
        return;
      }
  ...
  return (
    <form onSubmit={handleLogin} className={cn("flex flex-col gap-6", className)} {...props}>
      <div className="flex flex-col items-center gap-2 mb-1 text-center">
        ...
        <p className="text-muted-foreground text-sm text-balance">Enter your credentials to login</p>
      </div>
